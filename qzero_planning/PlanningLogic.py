...
class PlanningRepresentation:
    ...
    def __init__(self, machines, timesteps, domainactions, rewardstrategy, legal_actions, current_domainaction):
        ...
        self.schedule = np.zeros((self.machines, self.timesteps), dtype=int) # becomes the board
    ...
    def _valid_action(self, action, domainaction): #TODO: smt
        ...
        if time + duration < self.timesteps:
            for t in range(time, time+duration):
                if self.schedule[machine, t] != 0: # not occupied by another machine (domainaction)
                    return False
            return True
        return False
    ...
    def execute_move(self, action):
        ...
        duration = domainaction.duration
        for t in range(timestep, timestep+duration): # schedule the machine for the entire duration at once (non-premptive)
            self.schedule[machine,t] = domainaction.urn # machine number
            self.legal_actions.remove(self._move_to_action((machine, t))) # TODO: shouldn't this time step be removed for all the machines?
...
class RelativeProductRewardStrategy(RewardStrategy):
    ...
    # np.sum(row[:np.max(np.nonzero(row))+1] != 0) is 1 always
    # (np.max(np.nonzero(row))+1) is the number of free time slots before the last scheduled action per machine
    # taking a product and negative sign means that the ultimate goal is to minimize the free time slots before every machine
    def compute_reward(self, schedule):
